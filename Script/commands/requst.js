...
module.exports.handleReply = async function({ api, event, handleReply, getText }) {
  ...
  if (body.toLowerCase().startsWith("c") || body.toLowerCase().startsWith("cancel")) {
    ...
    if (indexes.length === 0) {
      return api.sendMessage("❌ Please provide thread numbers to reject!\nExample: c1 c2", threadID, messageID);
    }
    ...
    for (const num of indexes) {
      ...
      try {
        ...
        console.log(`Failed to reject ${handleReply.pending[index - 1].threadID}: ${e.message}`);
      }
    }
  ...
  // ✅ APPROVE
    ...
    if (indexes.length === 0) {
      return api.sendMessage("❌ Please provide thread numbers to approve!\nExample: 1 2 3", threadID, messageID);
    }
    ...
    for (const num of indexes) {
      ...
      try {
        // 🔥 APPROVE THE GROUP
        await api.unsendMessage(handleReply.messageID); // Clean up pending message
        await api.sendMessage(`✅ Group "${handleReply.pending[index - 1].name || 'Unnamed'}" approved successfully!`, threadID);
...
𝐎𝐰𝐧𝐞𝐫➢ 𝐌𝐑 𝐉𝐔𝐖𝐄𝐋`, groupID);
        ...
        console.log(`Failed to process ${groupID}: ${e.message}`);
      }
    }
...
module.exports.run = async function({ api, event, getText }) {
  ...
  try {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      msgArr.push(`┃ ${i + 1}. ${group.name || 'Unnamed'}
┃ 🆔 ${group.threadID}
┃ 👥 ${members} members
┃ 👤 ${addedByName}
┃ 🔗 ${profileLink}`);
    ...
    console.error("Request command error:", e);
